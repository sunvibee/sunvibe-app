...
class _HomeScreenState extends State<HomeScreen> with RouteAware {
  ...
  @override
  void didPopNext() {
    // Called when the top route has been popped off, and this route is now showing.
    // Reset the selected index to Home when returning to this screen
    if (mounted) {
      setState(() {
        selectedIndex = 0;
      });
  ...
  Future<bool> _onWillPop() async {
    // If we're already exiting, prevent double call
    if (_isExiting) return false;

    // Check if there are any screens in the navigation stack
    if (Navigator.of(context).canPop()) {
      // If there are screens above, just pop them
      Navigator.of(context).pop();
      return false;
    }

    // If we're at the root (Home screen), show exit dialog
    _isExiting = true;
