...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    super.initState();
    // Clear any leftover session data when login screen loads
    _clearSessionOnStart();
  }

  Future<void> _clearSessionOnStart() async {
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    // Only clear if not logged in (should already be logged out)
    if (!authProvider.isLoggedIn) {
      await authProvider.logout(); // Ensure clean state
    }
  }
  ...
  void login() async {
    ...
    if (robotId == validRobotId && password == validPassword) {
      // Save session using AuthProvider
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      await authProvider.login(
        robotId: robotId,
        userName: "SunVibee User",
      );
